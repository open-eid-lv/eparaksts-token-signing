...
GtkWidget *createPIN2Dialog(GtkWidget *window, GtkWidget *entry, const char *nameAndID, const char *message, const char *pinName, const unsigned minPin2Length) {
  ...
  GtkWidget *label = gtk_label_new(l10nReplacePIN2("For signing enter PIN2:", pinName));
...
GtkWidget *createPINPadDialog(GtkWidget *window, const char *nameAndID, const char *message, const char *pinName) {
  LOG_LOCATION;
  dialog = gtk_dialog_new_with_buttons(createDialogTitle(nameAndID), GTK_WINDOW(window),
  ...
  GtkWidget* label = gtk_label_new(l10nReplacePIN2("For signing enter PIN2 from PIN pad", pinName));
  gtk_label_set_use_underline(GTK_LABEL(label), TRUE);
...
char *promptForPIN(void *nativeWindowHandle, const char *name, const char *message, const char *pinName, unsigned minPin2Length, int usePinPad) {
  LOG_LOCATION;
  GtkWidget *window = getGtkWindow(nativeWindowHandle);
  ...
  if (!usePinPad) {
    // Simple card reader
    dialog = createPIN2Dialog(window, entry, name, message, pinName, minPin2Length);
    ...
    dialog = createPINPadDialog(window, name, message, pinName);
...
GtkWidget *createCertificateSelectionView(GtkWidget *window, GtkTreeModel* model) {
...
int promptForCertificate(void *nativeWindowHandle, char* certId) {
  ...
  GtkTreeModel* certModel = createAndFillModel();
  GtkWidget* certificatesView = createCertificateSelectionView(dialog, certModel);
    
  gint certCount = gtk_tree_model_iter_n_children(certModel, NULL);
  if (certCount == 0) {
      EstEID_log("No certificates");
      gtk_widget_destroy(dialog);
      return IDABORT;
  } else if (certCount == 1) {
      GtkTreeIter iter;
      gchar* id;
      gtk_tree_model_get_iter_first(certModel, &iter);
      gtk_tree_model_get(certModel, &iter, 3, &id, -1);
      strcpy(certId, id);
      g_free(id);
      EstEID_log("promptForCertificate dialog returned cert ID %s", certId);
      EstEID_log("Single signing cert");
      gtk_widget_destroy(dialog);
      return IDOK;
  }
