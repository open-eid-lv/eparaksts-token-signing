...
NPError NPP_New(NPMIMEType mimeType, NPP instanceData, uint16_t mode, int16_t argc, char* argn[], char* argv[], NPSavedData* saved) {
	EstEID_log("LOADED PLUGIN PKCS11 VERSION %s", ESTEID_PLUGIN_VERSION);
	...
	EstEID_log("Mac OSX x86_64 mode detected, dialogs are displayed in separate windows, not as sheets tied to browser window");
#endif
	...
	if (!EstEID_loadLibrary()) {
		if (!loadErrorShown) {
			NPVariant result;
			char code[2048];
			sprintf(code, "alert('eParaksts Plugin initialization failed:\\n%s');", EstEID_error);
