...
THREAD_RETURN_TYPE EstEID_pinPadLogin(void* threadData) {
	...
	pinPromptDataEx->pinPromptData.promptFunction(NULL, pinPromptDataEx->name, pinPromptDataEx->message, pinPromptDataEx->pinPromptData.pinName, 0, TRUE);
...
int EstEID_RealSign(CK_SESSION_HANDLE session, char **signature, unsigned int *signatureLength, const char *hash, unsigned int hashLength, char* name, EstEID_Map cert) {
	CK_OBJECT_HANDLE privateKeyHandle;
	CK_ULONG objectCount;
	unsigned int hashWithPaddingLength = 0;
	char *hashWithPadding;
    char *certificateId;
    int certificateIdLength;
	CK_MECHANISM mechanism = {CKM_RSA_PKCS, 0, 0};
    
    EstEID_log("Signing certificate id: %s", EstEID_mapGet(cert, "certificateIdHex"));
    
    certificateId = EstEID_hex2bin(EstEID_mapGet(cert, "certificateIdHex"));
    certificateIdLength = strlen(EstEID_mapGet(cert, "certificateIdHex")) / 2;
    
    CK_BYTE *objectId = (CK_BYTE *)certificateId;
    CK_OBJECT_CLASS keyClass = CKO_PRIVATE_KEY;
    CK_OBJECT_CLASS keyType = CKK_RSA;
    
    CK_ATTRIBUTE searchAttribute[] = {
        { CKA_CLASS, &keyClass, sizeof(keyClass) },
        { CKA_KEY_TYPE, &keyType, sizeof(keyType) },
        { CKA_ID, objectId, certificateIdLength * sizeof(CK_BYTE) }
    };

	if (EstEID_CK_failure("C_FindObjectsInit", fl->C_FindObjectsInit(session, searchAttribute, 3))) return FAILURE;
    
	if (EstEID_CK_failure("C_FindObjects", fl->C_FindObjects(session, &privateKeyHandle, 1, &objectCount))) return FAILURE;
	if (EstEID_CK_failure("C_FindObjectsFinal", fl->C_FindObjectsFinal(session))) return FAILURE;
    
	if (objectCount == 0) return FAILURE;
    
    EstEID_log("Private key handle: %04X", privateKeyHandle);
	
    if (EstEID_CK_failure("C_SignInit", fl->C_SignInit(session, &mechanism, privateKeyHandle))) return FAILURE;
...
int EstEID_sighHashWindows(char **signature, unsigned int *signatureLength, CK_SLOT_ID slotID, EstEID_Map cert, const char *hash, unsigned int hashLength, EstEID_PINPromptData pinPromptData) {
	...
	result = EstEID_RealSign(session, signature, signatureLength, hash, hashLength, NULL, cert);
    CLOSE_SESSION_AND_RETURN(result);
}
#endif
...
int EstEID_signHash(char **signature, unsigned int *signatureLength, CK_SLOT_ID slotID, EstEID_Map cert, const char *hash, unsigned int hashLength, EstEID_PINPromptData pinPromptData) {
	...
	for (attempt = 0;; attempt++) {
		...
		if (!remainingTries || blocked) {
			...
			pinPromptData.alertFunction(pinPromptData.nativeWindowHandle, l10nReplacePIN2("PIN2 blocked, cannot sign!", pinPromptData.pinName));
            CLOSE_SESSION_AND_RETURN(FAILURE);
		}
		if (remainingTries < 3 || attempt) {
			sprintf(message, "%s%s %i", (attempt ? l10nReplacePIN2("Incorrect PIN2! ", pinPromptData.pinName) : ""), l10n("Tries left:"), remainingTries);
		}
		else {
			message[0] = 0;
		}
		...
		if(!isPinPad) {
			// Simple card reader
			char *pin = pinPromptData.promptFunction(pinPromptData.nativeWindowHandle, name, message, pinPromptData.pinName, (unsigned)atoi(EstEID_mapGet(cert, "minPinLen")), isPinPad);
			if (!pin || strlen(pin) == 0) {
				free(pin);
			...
			pinPromptData.promptFunction(pinPromptData.nativeWindowHandle, name, message, pinPromptData.pinName,  0, isPinPad);
    ...
    result = EstEID_RealSign(session, signature, signatureLength, hash, hashLength, name, cert);
    CLOSE_SESSION_AND_RETURN(result);
}
...
int EstEID_signHashHex(char **signatureHex, CK_SLOT_ID slotID, EstEID_Map cert, const char *hashHex, EstEID_PINPromptData pinPromptData) {
    ...
    pinPromptData.pinName = (char*)EstEID_mapGet(cert, "pinName");
