...
int buildCertificatesList(HWND hwnd) {
	RECT rect;
...
INT_PTR CALLBACK CertSelectionDialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch(message) {
		case WM_INITDIALOG:
			...
			itemCount = buildCertificatesList(hwnd);
			selectedCertID = (char*)lParam;
			selectedCertID[0] = '\0';
            
			//ListView_GetItemCount(hwnd);
            
			EstEID_log("found certificates: %i", itemCount);
            
			if (itemCount == 0) {
				strcpy(selectedCertID, "");
				EstEID_log("no certificates dialog proc: %s", selectedCertID);
				INT_PTR r = IDABORT;
				EndDialog(hwnd, r);
			}
			else if (itemCount == 1) {
				TCHAR unicodeSelectedCertID[256];
				ListView_GetItemText(GetDlgItem(hwnd, IDC_CERTIFICATESLIST), 0, 3, (LPTSTR)unicodeSelectedCertID, 33);
				strcpy(selectedCertID, CW2A(unicodeSelectedCertID));
				EstEID_log("selected certificate id without dialog proc: %s", selectedCertID);
				INT_PTR r = IDOK;
				EndDialog(hwnd, r);
			}
			else {
