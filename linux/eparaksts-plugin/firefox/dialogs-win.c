...
void showAlert(void *nativeWindowHandle, const char *message) {
	WCHAR unicodeBufMessage[128];
	WCHAR unicodeBufTitle[128];
	MultiByteToWideChar(CP_UTF8, 0, message, -1, unicodeBufMessage, sizeof(unicodeBufMessage) / sizeof(WCHAR));
	MultiByteToWideChar(CP_UTF8, 0, l10n("Error"), -1, unicodeBufTitle, sizeof(unicodeBufTitle) / sizeof(WCHAR));
	
	//MessageBox((HWND)nativeWindowHandle, CA2W(message), CA2W(l10n("Error")), MB_OK | MB_ICONERROR);
	MessageBox((HWND)nativeWindowHandle, unicodeBufMessage, unicodeBufTitle, MB_OK | MB_ICONERROR);
}
...
CK_ULONG SetUpPin2Dialog(HWND hwnd, int retryCount, char* certId, const char* pinName) {
	char message[512];
	CK_SLOT_ID slotId;
	...
	if (remainingTries < 3 || retryCount > 0) {
		sprintf(message, "%s %i", l10n("Tries left:"), remainingTries);		
		sprintf(message, "%s%s %i", (retryCount ? l10nReplacePIN2("Incorrect PIN2! ", pinName) : ""), l10n("Tries left:"), remainingTries);
	}
	else {
		message[0] = 0;
	}
...
void EndDialogIfPIN2Blocked(HWND hwnd, int cardStatus, const char* pinName) {
	if(cardStatus == CKR_PIN_LOCKED) {
		ShowWindow(hwnd, false);
		showAlert(hwnd, l10nReplacePIN2("PIN2 blocked, cannot sign!", pinName));
...
INT_PTR CALLBACK Pin2DialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	switch(message) {
		case WM_INITDIALOG:
			pinName = (char *)((DialogData *)lParam)->pinName;
			...
			SetDialogItemText(dialogWindowHandle, IDC_PIN2_LABEL, l10nReplacePIN2("For signing enter PIN2:", pinName));
			...
			retryCount = 0;
			EndDialogIfPIN2Blocked(hwnd, SetUpPin2Dialog(hwnd, retryCount, certId, pinName), pinName);
			return true;
		case WM_COMMAND:
			...
			switch(LOWORD(wParam)) {
				case IDOK:
					...
					if(tmpSignature) {
						...
						cardStatus = SetUpPin2Dialog(hwnd, retryCount, certId, pinName);
						if( cardStatus!= CKR_OK) {
							EndDialogIfPIN2Blocked(hwnd, cardStatus, pinName);
						}
						return true;
					}
				case IDCANCEL:
...
INT_PTR CALLBACK PinPadDialogProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	switch(message) {
		case WM_INITDIALOG:
			...
			SetDialogItemText(hwnd, IDC_PINPAD_PIN2_LABEL, l10nReplacePIN2("For signing enter PIN2 from PIN pad", ((DialogData *)lParam)->pinName));
...
char *promptForPIN(void *nativeWindowHandle, const char *name, const char *message, const char *pinName, unsigned int minPin2Length, int usePinPad) {
	LOG_LOCATION;
