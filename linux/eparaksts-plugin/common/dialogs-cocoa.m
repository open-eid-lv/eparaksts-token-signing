...
char *cocoa_promptForPIN(NSWindow *window, const char *name, const char *message, const char *pinName, unsigned minPin2Length, int usePinPad) {
	...
	char *pin = strdup([[pinPanel showForWindow: window withName: name withMessage: message  withPinName:pinName withMinAcceptablePin2Length: minPin2Length usePinPad: usePinPad] UTF8String]);
	LOG_LOCATION;
	[pool release];	
	LOG_LOCATION;
	return pin;
...
int cocoa_promptForCertificate(void* nativeWindowHandle, char* certId) {
    ...
    int certCount = [[certificateSelection certificates] count];
    ...
    if (certCount > 1) {
        ...
        strcpy(certId, [[[[certificateSelection certificates] objectAtIndex:0] objectForKey:@"id"] UTF8String]);
        returnValue = IDOK;
    } else {
        strcpy(certId, "");
        returnValue = IDABORT;
