...
#include "eParakstsCertificate.h"
#include "EstEIDHelper.h"
extern "C" {
#include "esteid_log.h"
}

STDMETHODIMP CeParakstsCertificate::get_id(BSTR *id){
	EstEID_log("");
	*id = _bstr_t(this->id.c_str()).Detach();
	return S_OK;
}

void CeParakstsCertificate::readFromCertContext() {
	LOG_LOCATION;
...
void CeParakstsCertificate::loadCertContexts(PCCERT_CONTEXT certContext) {
	USES_CONVERSION;
	EstEID_log("");
...
STDMETHODIMP CeParakstsCertificate::get_certificateAsHex(BSTR *certificate) {
	EstEID_log("");
	*certificate = _bstr_t(this->certificateAsHex.data()).Detach();
	return S_OK;
}

STDMETHODIMP CeParakstsCertificate::get_cert(BSTR *certificate) {
	EstEID_log("");
	*certificate = _bstr_t(this->certificateAsHex.data()).Detach();
	return S_OK;
}

void CeParakstsCertificate::binCert2Hex(const unsigned int binLength) {
	EstEID_log("");
...
void CeParakstsCertificate::calculateMD5Hash(unsigned int certLength) {
	EstEID_log("");
