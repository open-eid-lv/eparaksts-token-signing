...
#include "stdafx.h"
#include "eParakstsIEPluginBHO.h"
...
STDMETHODIMP CeParakstsIEPluginBHO::SetSite(IUnknown* pUnkSite) {
	EstEID_log("");
	IObjectWithSiteImpl<CeParakstsIEPluginBHO>::SetSite(pUnkSite);
...
STDMETHODIMP CeParakstsIEPluginBHO::get_version(BSTR *result) {
	EstEID_log("");
	*result = _bstr_t(ESTEID_PLUGIN_VERSION).Detach();
	return S_OK;
}

STDMETHODIMP CeParakstsIEPluginBHO::get_pluginLanguage(BSTR *result) {
	EstEID_log("");
...
STDMETHODIMP CeParakstsIEPluginBHO::put_pluginLanguage(BSTR language) {
	EstEID_log("");
...
STDMETHODIMP CeParakstsIEPluginBHO::get_errorMessage(BSTR *result) {
	EstEID_log("");
	*result = _bstr_t(this->errorMessage.c_str()).Detach();
	return S_OK;
}

STDMETHODIMP CeParakstsIEPluginBHO::get_errorCode(BSTR *result) {
	EstEID_log("");
	*result = _bstr_t(this->errorCode).Detach();
	return S_OK;
}

BOOL CeParakstsIEPluginBHO::isSiteAllowed() {
	EstEID_log("");
	BSTR url_buffer;
	if(webBrowser == NULL){
...
STDMETHODIMP CeParakstsIEPluginBHO::getCertificate(IDispatch **_certificate){
	EstEID_log("");
	FAIL_IF_SITE_IS_NOT_ALLOWED;
	try {
		if(!this->certificate || !isSameCardInReader(this->certificate)) {
			this->certificate.CoCreateInstance(CLSID_eParakstsCertificate);
		}
		CComPtr<IeParakstsCertificate> cert;
...
BOOL CeParakstsIEPluginBHO::isSameCardInReader(CComPtr<IeParakstsCertificate> _cert){ //todo: must check is card changed
	/* not implemented yet */
	this->certificate = NULL;
	return false;
}

STDMETHODIMP CeParakstsIEPluginBHO::sign(BSTR id, BSTR hash, BSTR language, BSTR *signature){
	LOG_LOCATION;
	FAIL_IF_SITE_IS_NOT_ALLOWED;
	USES_CONVERSION;
	try {
...
void CeParakstsIEPluginBHO::clearErrors() {
	EstEID_log("");
	this->errorCode = 0;
	this->errorMessage.assign("");
}

void CeParakstsIEPluginBHO::setError(BaseException &exception) {
	EstEID_log("");
