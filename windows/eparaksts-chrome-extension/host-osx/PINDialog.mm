...
- (instancetype)init:(BOOL)pinpad pinName:(NSString *)pinName
{
    if (self = [super init]) {
        ...
        NSString *pinLabel = _L(pinpad ? "enter PIN2 pinpad" : "enter PIN2");
        pinFieldLabel.stringValue = [pinLabel stringByReplacingOccurrencesOfString:@"PIN2" withString:pinName];
...
+ (NSDictionary *)show:(NSDictionary*)params cert:(NSString *)cert
{
    ...
    try {
        for (auto &token : PKCS11CardManager::instance()->getAvailableTokens()) {
            ...
            if (selected->haveSigningCertificate()) {
    ...
    NSString *pinName = [NSString stringWithUTF8String:selected->getPINName()];

    for (int retriesLeft = selected->getPIN2RetryCount(); retriesLeft > 0; ) {
        PINPanel *dialog = [[PINPanel alloc] init:selected->isPinpad() pinName:pinName];
        if (!dialog) {
            return @{@"result": @"technical_error"};
        }
        ...
        if (retriesLeft < 3) {
            NSString * pinLabel =  [NSString stringWithFormat:@"%@%@ %u", (isInitialCheck ? @"" : _L("incorrect PIN2")), _L("tries left"), retriesLeft];
            dialog->messageField.stringValue = [pinLabel stringByReplacingOccurrencesOfString:@"PIN2" withString:pinName];
        }
...
- (void)controlTextDidChange:(NSNotification*)notification;
{
    ...
    pinField.stringValue = [[pinField.stringValue componentsSeparatedByCharactersInSet:
                             ((NSCharacterSet*)NSCharacterSet.decimalDigitCharacterSet).invertedSet]
