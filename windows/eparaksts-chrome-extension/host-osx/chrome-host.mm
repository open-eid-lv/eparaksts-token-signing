...
static void write(NSDictionary *data, NSString *nonce)
{
    ...
    [((NSFileHandle*)NSFileHandle.fileHandleWithStandardOutput) writeData:sizeout];
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        [dc addObserverForName:NSFileHandleDataAvailableNotification object:input queue:nil usingBlock:^(NSNotification *note) {
            ...
            {
                uint32_t size = 0;
                NSData *length = [input readDataOfLength:sizeof(size)];
                [length getBytes:&size];
                ...
                NSData *json = [input readDataOfLength:size];;
                //
